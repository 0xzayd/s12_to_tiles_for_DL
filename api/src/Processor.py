...
class Processor(object):
    ...
    def merge(self):
        self.output_folder = os.path.join(self.zips_path, '..')
        file_string = os.path.basename(os.path.dirname(self.zips_path)) + self.suffix
        if self.suffix == 'S1':        
            if len(self.paths_to_merge)>1:
                merge_rasters(self.paths_to_merge, self.output_folder, file_string, self.dtype)
            elif len(self.paths_to_merge) == 1:
                shutil.copy(self.paths_to_merge[0], self.output_folder + '/{0}.tif'.format(file_string))
            ...
            if len(self.paths_to_merge) > 1:
                for key in self.paths_to_merge[0].keys():
                    ...
                    merge_rasters(list_bands_to_merge, self.output_folder, file_string2, self.dtype)

            elif len(self.paths_to_merge) == 1:
                for key in self.paths_to_merge[0].keys():
                    file_string2 = '{0}_'.format(key) + file_string
                    shutil.copy(self.paths_to_merge[0][key], self.output_folder + '/{0}.tif'.format(file_string2))
